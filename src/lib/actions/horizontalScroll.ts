...
interface HorizontalScrollOptions {
	/**
	 * Multiplier to be applied to the caught wheel event's `deltaY` when transposing to
	 * `scrollLeft`.
	 */
	multiplier?: number;
	/**
...
export function horizontalScroll(
	element: HTMLElement,
	{ multiplier = 1, precedeVertical = true }: HorizontalScrollOptions = {}
) {
	function handleScroll(e: Event) {
		if (!(e instanceof WheelEvent) || isTouchpad(e) || e.deltaX) return;
